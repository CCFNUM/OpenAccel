"""Generate a 2D box mesh for the Euler-Euler bubble-column case.

The Exodus II mesh has one QUAD4 element block named ``fluid`` and four side
sets: no-slip ``wall_l``/``wall_r``, ``inlet``, and ``outlet``.
"""
import netCDF4 as nc
import numpy as np

NX, NY = int(__import__("sys").argv[1]), int(__import__("sys").argv[2])
LX, LY = 0.15, 1.0
...
sidesets = {
    "wall_l": [(eid(0, j), 4) for j in range(NY)],
    "wall_r": [(eid(NX - 1, j), 2) for j in range(NY)],
    "inlet":  [(eid(i, 0), 1) for i in range(NX)],
    "outlet": [(eid(i, NY - 1), 3) for i in range(NX)],
}
...
d.title = "bubbleColumnLaminar mesh"
