...
dgInterfaceSideInfo::dgInterfaceSideInfo(
    interface* interfPtr,
    bool isMasterSide,
      ...
      dynamicSearchTolAlg_(dynamicSearchTolAlg), useShifted_(useShifted)
{
    if (messager::master())
    {
        std::cout << "\t[DG Init] Side: " << name_
                  << " | Search tol: " << searchTolerance_ << std::endl;
    }
}
...
void dgInterfaceSideInfo::deleteDgInfo()
{
    // Storage is owned by the base interfaceSideInfo.
    clearIpInfo_();
}

void dgInterfaceSideInfo::setup()
{
}

void dgInterfaceSideInfo::initialize()
{
...
void dgInterfaceSideInfo::update()
{
...
void dgInterfaceSideInfo::reset()
{
    // clear all search related info
...
void dgInterfaceSideInfo::constructDgInfo()
{
         ...
         ++ib)
    {
        ...
        for (stk::mesh::Bucket::size_type k = 0; k < length; ++k)
        {
            ...
            std::vector<ipInfo*> faceIpInfoVec(numScsBip);
            for (label ip = 0; ip < numScsBip; ++ip)
            {
                dgInfo* dgInfo_ptr = new dgInfo(messager::myProcNo(),
                                                ...
                                                currentElemTopo,
                                                searchTolerance_);
                faceIpInfoVec[ip] = dgInfo_ptr;
            }

            // push them all back into the base storage
            ipInfoVec_.push_back(std::move(faceIpInfoVec));
        }
    }
}

void dgInterfaceSideInfo::constructBoundingPoints()
{
    ...
    for (auto& faceIpInfoVec : ipInfoVec_)
    {
        //=======================================================
        // all ips on this face use a common face master element
        //            gather common operations once
        //=======================================================
        const dgInfo* firstDgInfo =
            static_cast<const dgInfo*>(faceIpInfoVec[0]);
        ...
        {
            dgInfo* dgInfo = static_cast<accel::dgInfo*>(faceIpInfoVec[k]);
...
void dgInterfaceSideInfo::constructBoundingBoxes()
{
    const auto& bulkData = interfPtr_->meshRef().bulkDataRef();
    const auto& metaData = interfPtr_->meshRef().metaDataRef();
...
void dgInterfaceSideInfo::doSearch_()
{
...
void dgInterfaceSideInfo::repeatSearchIfNeeded_(
    ...
    std::vector<std::pair<theKey, theKey>>& searchKeyPair) const
{
    ...
    while (any_not_empty)
    {
        ...
        if (10 < num_iterations)
        {
            if (messager::master())
                std::cout
                    << "dgInterfaceSideInfo::repeatSearchIfNeeded issue with "
                    << name_ << std::endl;
            if (messager::master())
...
void dgInterfaceSideInfo::deleteRangePointsFound_(
...
void dgInterfaceSideInfo::determineElemsToGhost()
{
    const auto& bulkData = interfPtr_->meshRef().bulkDataRef();
    const auto& metaData = interfPtr_->meshRef().metaDataRef();
...
void dgInterfaceSideInfo::completeSearch()
{
    const auto& bulkData = interfPtr_->meshRef().bulkDataRef();
    const auto& metaData = interfPtr_->meshRef().metaDataRef();
    ...
    // invert the process... Loop over the base ipInfoVec_ and query
    // searchKeyPair_ for this information
    ...
    for (size_t iv = 0; iv < ipInfoVec_.size(); ++iv)
    {
        std::vector<ipInfo*>& theVec = ipInfoVec_[iv];
        for (size_t k = 0; k < theVec.size(); ++k)
        {
            // extract the info object; new and old
            dgInfo* dgInfo = static_cast<accel::dgInfo*>(theVec[k]);
...
void dgInterfaceSideInfo::determineOpposingGaussPointIds()
{
    for (auto& faceIpInfoVec : ipInfoVec_)
    {
        for (ipInfo* ip : faceIpInfoVec)
        {
            dgInfo* dg = static_cast<dgInfo*>(ip);
...
void dgInterfaceSideInfo::provideDiagnosis()
{
    const auto& bulkData = interfPtr_->meshRef().bulkDataRef();
    const auto& metaData = interfPtr_->meshRef().metaDataRef();
...
void dgInterfaceSideInfo::dumpSearchResults()
{
...
void dgInterfaceSideInfo::dumpFaceToFaceResults()
{
    const auto& bulkData = interfPtr_->meshRef().bulkDataRef();
    ...
    for (label proci = 0; proci < messager::nProcs(); proci++)
    {
        if (proci == messager::myProcNo())
        {
            for (auto& faceIpInfoVec : ipInfoVec_)
            {
                ...
                {
                    dgInfo* dgInfo =
                        static_cast<accel::dgInfo*>(faceIpInfoVec[k]);
...
size_t dgInterfaceSideInfo::errorCheck()
{
