// File : correlationTransitionShearStressTransportModel.cpp
...
correlationTransitionShearStressTransportModel::
    correlationTransitionShearStressTransportModel(realm* realm)
    : shearStressTransportModel(realm)
{
    // Create field instances:
    // Only gamma is needed for the correlation-based model
    // (No ReTheta transport equation)
    gammaRef();
}

scalar correlationTransitionShearStressTransportModel::FPG(scalar lambda0L,
                                                           scalar CPG1,
                                                           scalar CPG2,
                                                           scalar CPG3,
                                                           scalar CPG1_lim,
                                                           scalar CPG2_lim)
{
    ...
    if (lambda0L >= 0.0)
    {
        out = std::min(1.0 + CPG1 * lambda0L, CPG1_lim);
    }
    else
    {
        out = std::min(1.0 + CPG2 * lambda0L +
                           CPG3 * std::min(lambda0L + 0.0681, 0.0),
                       CPG2_lim);
    }
...
void correlationTransitionShearStressTransportModel::updateFOneBlending(
    const std::shared_ptr<domain> domain)
{
    // compute F1 with parameters appropriate for 2003 SST implementation
    // with transition modification (F3 term)
         ...
         ++ib)
    {
        ...
        for (stk::mesh::Bucket::size_type k = 0; k < length; ++k)
        {
            ...
            // Transition SST Modification (same as full transition model)
...
void correlationTransitionShearStressTransportModel::updateTurbulentProduction(
    const std::shared_ptr<domain> domain)
{
    stk::mesh::BulkData& bulkData = this->meshRef().bulkDataRef();
    stk::mesh::MetaData& metaData = this->meshRef().metaDataRef();
