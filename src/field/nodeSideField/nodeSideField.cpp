                                           ...
                                           bool master)
{
    ...
    // zero-out side field on the current boundary parts
    this->setToValue({0.0}, interfaceSideInfoPtr->currentPartVec_);
    ...
    const auto& exposedAreaVecSTKFieldRef = *metaData.get_field<scalar>(
        ...
        this->getExposedAreaVectorID_(
            interfaceSideInfoPtr->zonePtr()->index()));
    ...
    // define field on boundary parts if not yet
    for (const stk::mesh::Part* part : interfaceSideInfoPtr->currentPartVec_)
    {
        if (!(localAssembledAreaSTKFieldPtr->defined_on(*part)))
        {
            // put on all parts which the current node-side field is defined on
    ...
    if (interfaceSideInfoPtr->zonePtr()->meshDeforming())
    {
        areaCalc = true;
    }
    ...
    bool master)
{
    ...
    // zero-out side field on the current boundary parts
    this->setToValue(std::vector<scalar>(SPATIAL_DIM, 0.0),
