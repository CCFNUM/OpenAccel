// File : bulkPressureCorrectionAssembler.cpp
...
bulkPressureCorrectionAssembler::bulkPressureCorrectionAssembler(
    freeSurfaceFlowModel* model)
    : phiAssembler<1>(static_cast<fieldBroker*>(model)), model_(model)
{
}

void bulkPressureCorrectionAssembler::adjustMatrixForPressureReference(
    const domain* domain,
    Context* ctx)
{
