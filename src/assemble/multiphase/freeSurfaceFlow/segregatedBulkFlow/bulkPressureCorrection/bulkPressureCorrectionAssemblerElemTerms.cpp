// File : bulkPressureCorrectionAssemblerElemTerms.cpp
...
void bulkPressureCorrectionAssembler::assembleElemTermsInterior_(
    const domain* domain,
    Context* ctx)
{
    ...
    const auto& betaRhoSTKFieldRef =
        model_->rhoRef(phaseIndex_).blendingFactorRef().stkFieldRef();
    const auto& gradRhoSTKFieldRef =
        model_->rhoRef(phaseIndex_).gradRef().stkFieldRef();
    ...
    const auto& mDotSTKFieldRef = model_->mDotRef(phaseIndex_).stkFieldRef();
             ...
             ++iElement)
        {
            ...
            for (label ip = 0; ip < numScsIp; ++ip)
            {
                ...
                for (label ic = 0; ic < nodesPerElement; ++ic)
                {
                    ...
                    p_lhs[rowL + ic] += lhsfac * alphaIp / densityScale;

                    // assemble to lhs; right
                    p_lhs[rowR + ic] -= lhsfac * alphaIp / densityScale;
                }
                ...
                p_lhs[rLiL_i] += alhsfacL * alphaIp / densityScale;
                p_lhs[rRiL_i] -= alhsfacL * alphaIp / densityScale;

                // upwind advection right node: will be 0 for incompressible
                // flow
                const scalar alhsfacR =
                    0.5 * (tmDot - std::abs(tmDot)) * psiUpwind / rhoHR * comp;
                p_lhs[rRiR_i] -= alhsfacR * alphaIp / densityScale;
                ...
                p_rhs[il] -= mDot * alphaIp / densityScale;
                p_rhs[ir] += mDot * alphaIp / densityScale;
            }
