...
bool assembleFEMInterfacePenalty(const label currentNodes,
                                 ...
                                 scalar* currentOpposingTangent)
{
    if (currentNodes <= 0 || opposingNodes <= 0 || !currentShape ||
        ...
        !opposingDisplacement || !currentResidual || !currentCurrentTangent ||
    ...
    for (label row = 0; row < SPATIAL_DIM; ++row)
    {
        ...
        {
            jump[row] -= opposingToCurrent[row * SPATIAL_DIM + column] *
    ...
    {
        for (label rowComponent = 0; rowComponent < SPATIAL_DIM; ++rowComponent)
        {
            const label row = currentTest * SPATIAL_DIM + rowComponent;
            ...
            for (label currentTrial = 0; currentTrial < currentNodes;
                 ++currentTrial)
            {
                const label column = currentTrial * SPATIAL_DIM + rowComponent;
            ...
            for (label opposingTrial = 0; opposingTrial < opposingNodes;
                 ++opposingTrial)
            {
                for (label columnComponent = 0; columnComponent < SPATIAL_DIM;
                     ++columnComponent)
                {
                    const label column =
    ...
    Context* ctx)
{
    if (field_broker_->controlsRef().isCvfemSolidMechanics())
    {
        ...
        else
        {
            ...
            // preserve the original errorMsg behavior at the top of the
            // function and run the unified loop for DG below.

            // Unified loop over per-IP info.  Storage is owned by the base
            // interfaceSideInfo; concrete side classes store derived records
            // upcast to ipInfo*, and ip->areaFraction_ is the default 1.0 so
            // the math is identical to the original DG implementation.
            for (auto& faceIpInfoVec : interfaceSideInfoPtr->ipInfoVec())
            {
                for (size_t k = 0; k < faceIpInfoVec.size(); ++k)
                {
                    ...
                    ws_o_elem_disp.resize(opposingNodesPerElement *
                                          SPATIAL_DIM);
                    ...
                    for (label ni = 0; ni < current_num_face_nodes; ++ni)
                    {
                        ...
                        const scalar E =
                            *stk::mesh::field_data(ESTKFieldRef, node);
                        const scalar nu =
                    ...
                    for (label ni = 0; ni < current_num_elem_nodes; ++ni)
                    {
                        ...
                        for (label i = 0; i < SPATIAL_DIM; ++i)
                        {
                            p_c_elem_coordinates[ni * SPATIAL_DIM + i] =
                                coords[i];
                        }
                    }
                    ...
                    for (label ni = 0; ni < opposing_num_elem_nodes; ++ni)
                    {
                        ...
                        for (label i = 0; i < SPATIAL_DIM; ++i)
                        {
                            p_o_elem_coordinates[ni * SPATIAL_DIM + i] =
                                coords[i];
                        }
                    }
                    ...
                    for (label i = 0; i < SPATIAL_DIM; ++i)
                    {
                        ...
                        const scalar ncPenalty =
                            penaltyIp *
                            (currentDispBip[i] - opposingDispBip[i]);
    ...
    else
    {
        ...
        if (interfaceSideInfoPtr->interfPtr()->isFluidSolidType())
        {
            ...
            for (const stk::mesh::Bucket* bucket : sideBuckets)
            {
                ...
                const label integrationPoints =
                    faceMasterElement->numIntPoints_;
        ...
        for (const auto& faceIpInfoVec : interfaceSideInfoPtr->ipInfoVec())
        {
            for (const ipInfo* ip : faceIpInfoVec)
            {
                ...
                const label opposingFaceNodes =
                    opposingFaceME->nodesPerElement_;
                ...
                opposingFaceDisplacement.resize(opposingFaceNodes *
                                                SPATIAL_DIM);
                ...
                for (label node = 0; node < currentFaceNodes; ++node)
                {
                    const scalar E = *stk::mesh::field_data(
                        EField, currentFaceNodeRels[node]);
                    const scalar nu = *stk::mesh::field_data(
                        nuField, currentFaceNodeRels[node]);
                ...
                {
                    const scalar E = *stk::mesh::field_data(
                        EField, opposingFaceNodeRels[node]);
                    const scalar nu = *stk::mesh::field_data(
                        nuField, opposingFaceNodeRels[node]);
                ...
                {
                    const scalar area =
                        areaVector[ip->currentGaussPointId_ * SPATIAL_DIM +
                                   dim];
                    areaMagnitudeSquared += area * area;
                ...
                const bool interfaceStatus =
                    assembleFEMInterfacePenalty(currentFaceNodes,
                                                opposingFaceNodes,
                                                currentShape.data(),
                                                opposingShape.data(),
                                                opposingToCurrent,
                                ...
                                const label elementColumn =
                                    (currentElementNodes +
                                     opposingTrialElementNode) *
                                        SPATIAL_DIM +
                                    columnComponent;
                                lhs[elementRow * totalDofs +
                                    elementColumn] += currentOpposingTangent
