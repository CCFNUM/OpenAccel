    ...
    std::vector<scalar>& lhs)
{
    ...
    for (const scalar xi : gaussPoints)
    {
        ...
        {
            scalar dN_dxi[4] = {-0.25 * (1.0 - eta),
                                0.25 * (1.0 - eta),
                                0.25 * (1.0 + eta),
                                -0.25 * (1.0 + eta)};
            scalar dN_deta[4] = {-0.25 * (1.0 - xi),
                                 -0.25 * (1.0 + xi),
                                 0.25 * (1.0 + xi),
                                 0.25 * (1.0 - xi)};
            ...
            for (label node = 0; node < 4; ++node)
            {
                dN_dx[node] = invJ00 * dN_dxi[node] + invJ10 * dN_deta[node];
                dN_dy[node] = invJ01 * dN_dxi[node] + invJ11 * dN_deta[node];
            }
            ...
            for (label a = 0; a < 4; ++a)
            {
                ...
                for (label b = 0; b < 4; ++b)
                {
                    ...
                    const scalar kxx = Bax[0] * (c11 * Bbx[0] + c12 * Bbx[1]) +
                                       Bax[1] * (c12 * Bbx[0] + c11 * Bbx[1]) +
                                       Bax[2] * c33 * Bbx[2];
                    const scalar kxy = Bax[0] * (c11 * Bby[0] + c12 * Bby[1]) +
                                       Bax[1] * (c12 * Bby[0] + c11 * Bby[1]) +
                                       Bax[2] * c33 * Bby[2];
                    const scalar kyx = Bay[0] * (c11 * Bbx[0] + c12 * Bbx[1]) +
                                       Bay[1] * (c12 * Bbx[0] + c11 * Bbx[1]) +
                                       Bay[2] * c33 * Bbx[2];
                    const scalar kyy = Bay[0] * (c11 * Bby[0] + c12 * Bby[1]) +
    ...
    Context* ctx)
{
    if (field_broker_->controlsRef().isCvfemSolidMechanics())
    {
        ...
        // Get the solid mechanics option (linear elastic vs simplified
        // neo-hookean)
                 ...
                 ++iElement)
            {
                ...
                for (label ip = 0; ip < numScsIp; ++ip)
                {
                        ...
                        for (label ic = 0; ic < nodesPerElement; ++ic)
                        {
                            ...
                            for (label i = 0; i < SPATIAL_DIM; ++i)
                            {
                                ...
                                for (label j = 0; j < SPATIAL_DIM; ++j)
                                {
                                    ...
                                    // Lambda divergence term: -lambda *
                                    // delta_ij * dN/dx_l * A_j
                             ...
                             solidMechanicsOption::neoHookean)
                    {
                        ...
                        {
                            J = p_gradTens[0] *
                                    (p_gradTens[4] * p_gradTens[8] -
                                     p_gradTens[7] * p_gradTens[5]) -
                                p_gradTens[1] *
                                    (p_gradTens[3] * p_gradTens[8] -
                        ...
                        for (label ic = 0; ic < nodesPerElement; ++ic)
                        {
                            ...
                            for (label i = 0; i < SPATIAL_DIM; ++i)
                            {
                                ...
                                for (label j = 0; j < SPATIAL_DIM; ++j)
                                {
                                    ...
                                    // First set of terms: mu*beta/J * F_ik *
                                    // dN/dxk
                                    // * Aj
                                    ...
                                    // Second set of terms: mu*beta/J * dN/dxj *
                                    // Aj
                                    const scalar lhsfacDiff_j =
                                        -((muIp * beta) / J) *
                                    ...
                                    // Third set of terms: mu*beta/J * dN/dxi *
                                    // Aj
                                    const scalar lhsfacDiff_i =
                                        -((muIp * beta) / J) *
                                ...
                                // Accumulated diagonal term
                                p_lhs[indexL * rhsSize + icNdim + i] +=
                                    lhs_riC_i;
                                p_lhs[indexR * rhsSize + icNdim + i] -=
                                    lhs_riC_i;
                                p_rhs[indexL] -= lhs_riC_i * dxi;
                                p_rhs[indexR] += lhs_riC_i * dxi;
                            }
    ...
    else
    {
        ...
        // switch so the lambda term follows the same plane_stress selection as
        // the CVFEM solid path.
        ...
        for (const stk::mesh::Bucket* bucket : elementBuckets)
        {
            ...
            for (stk::mesh::Entity elem : *bucket)
            {
                ...
                {
#if SPATIAL_DIM == 2
                    // sfem provides the isoparametric matrix-free operator.
                    // Applying it to each local basis vector recovers the
                    // element matrix while OpenAccel retains ownership of the
                    // global block matrix.
#endif
                    ...
                    messager::myProcNo() == 0)
                {
                    ...
                    for (label row = 0; row < dofsPerElement; ++row)
                    {
                        const scalar diagonal = lhs[row * dofsPerElement + row];
