...
void turbulentIntermittencyCorrelationTransitionSSTEquation::checkDomain(
    const std::shared_ptr<domain> domain)
{
    assert(domain->type() == domainType::fluid);
}

bool turbulentIntermittencyCorrelationTransitionSSTEquation::isConverged() const
{
    return linearSystem::isConverged();
}

void turbulentIntermittencyCorrelationTransitionSSTEquation::setup()
{
    ...
    assembler_->setup(&model_->gammaRef(),
                      advectionDiffusion,
                      domainVector_,
                      // anonymous function to compute Gamma for gamma equation:
                      [this](const domain* domain, STKScalarField& Gamma)
    {
        ...
        // Get sigma_gamma from the correlation-based transition model
        const scalar sigmaGamma = model_->sigmaGamma();
        ...
        for (size_t ib = 0; ib < nodeBuckets.size(); ib++)
        {
            ...
            {
                Gammab[i] = mu[i] + mut[i] / sigmaGamma;
            }
        }
    });
...
void turbulentIntermittencyCorrelationTransitionSSTEquation::initialize()
{
...
void turbulentIntermittencyCorrelationTransitionSSTEquation::postInitialize()
{
}

void turbulentIntermittencyCorrelationTransitionSSTEquation::preSolve()
{
    // raw update
...
void turbulentIntermittencyCorrelationTransitionSSTEquation::solve()
{
...
void turbulentIntermittencyCorrelationTransitionSSTEquation::preTimeStep()
{
    FOREACH_DOMAIN(model_->updateTurbulentIntermittencyPrevTimeField);
}

void turbulentIntermittencyCorrelationTransitionSSTEquation::
    setResidualScales_()
{
...
void turbulentIntermittencyCorrelationTransitionSSTEquation::
    applyDependencyUpdates_(const domain* domain,
...
void turbulentIntermittencyCorrelationTransitionSSTEquation::applyOverrides()
{
