...
public:
    ...
    ViewType
    copy_interpolation_weights_to_view(const std::vector<double>& interps)
    {
...
protected:
    ...
    double tensor_product_weight(int s1Node,
                                 int s2Node,
                                 int s3Node,
                                 int s1Ip,
                                 int s2Ip,
    ...
    void hex27_shape_deriv(int npts,
...
class Hex27SCV : public HexahedralP2Element
{
    ...
    using GradWeightType = AlignedViewType<
        DoubleType[AlgTraits::numScvIp_][AlgTraits::nodesPerElement_]
                  [AlgTraits::nDim_]>;
...
public:
    ...
    void determinant(SharedMemView<DoubleType**>& coords,
                     SharedMemView<DoubleType*>& volume) final;
    ...
    template <typename GradViewType,
              typename CoordViewType,
              typename OutputViewType>
    void weighted_volumes(GradViewType referenceGradWeights,
...
private:
    ...
    double
    jacobian_determinant(const double* POINTER_RESTRICT elemNodalCoords,
...
class Hex27SCS : public HexahedralP2Element
{
    ...
    using GradWeightType = AlignedViewType<
        DoubleType[AlgTraits::numScsIp_][AlgTraits::nodesPerElement_]
                  [AlgTraits::nDim_]>;
    using ExpGradWeightType = AlignedViewType<
        DoubleType[6 * AlgTraitsQuad9Hex27::numFaceIp_]
...
public:
    ...
    template <typename GradViewType,
              typename CoordViewType,
              typename OutputViewType>
    void weighted_area_vectors(GradViewType referenceGradWeights,
                               CoordViewType coords,
                               OutputViewType areav)
    {
        ...
        static_assert(OutputViewType::rank == 2,
                      "area_vector view assumed to be 2D");

        static_assert(AlgTraits::numScsIp_ % AlgTraits::nDim_ == 0,
                      "Number of ips incorrect");
...
private:
    ...
    void gradient(const double* POINTER_RESTRICT elemNodalCoords,
...
private:
    ...
    void non_unit_face_normal(const double* isoParCoord,
