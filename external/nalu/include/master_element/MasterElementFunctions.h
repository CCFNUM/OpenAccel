...
inline void cofactorMatrix(ftype adjJac[][3], const ftype jact[][3])
{
...
template <typename ftype>
inline void cofactorMatrix(ftype adjJac[][2], const ftype jact[][2])
{
    adjJac[0][0] = jact[1][1];
    adjJac[0][1] = -jact[1][0];
    adjJac[1][0] = -jact[0][1];
...
void generic_grad_op(const GradViewType& referenceGradWeights,
                     ...
                     OutputViewType& weights)
{
    ...
    static_assert(CoordViewType::rank == 2,
                  "Coordinate view assumed to be rank 2");
    static_assert(OutputViewType::rank == 3,
                  "Weight view assumed to be rank 3");

    STK_ThrowAssert(AlgTraits::nodesPerElement_ ==
    ...
    for (unsigned ip = 0; ip < referenceGradWeights.extent(0); ++ip)
    {
        ...
        STK_ThrowAssertMsg(stk::simd::are_any(det > tiny_positive_value()),
...
template <typename AlgTraits,
          ...
          typename OutputViewType,
          typename DetjType>
void generic_grad_op_fem(const GradViewType& referenceGradWeights,
                         const CoordViewType& coords,
                         OutputViewType& weights,
                         DetjType& detj)
{
...
template <typename AlgTraits,
          ...
          typename CoordViewType,
          typename OutputViewType>
void generic_gij_3d(const GradViewType& referenceGradWeights,
                    const CoordViewType& coords,
                    OutputViewType& gup,
                    OutputViewType& glo)
{
    ...
    for (unsigned ip = 0; ip < referenceGradWeights.extent(0); ++ip)
    {
        ...
        gup(ip, 0, 0) = jac[0][0] * jac[0][0] + jac[0][1] * jac[0][1] +
                        jac[0][2] * jac[0][2];
        gup(ip, 0, 1) = jac[0][0] * jac[1][0] + jac[0][1] * jac[1][1] +
                        jac[0][2] * jac[1][2];
        gup(ip, 0, 2) = jac[0][0] * jac[2][0] + jac[0][1] * jac[2][1] +
                        jac[0][2] * jac[2][2];

        gup(ip, 1, 0) = gup(ip, 0, 1);
        gup(ip, 1, 1) = jac[1][0] * jac[1][0] + jac[1][1] * jac[1][1] +
                        jac[1][2] * jac[1][2];
        gup(ip, 1, 2) = jac[1][0] * jac[2][0] + jac[1][1] * jac[2][1] +
                        jac[1][2] * jac[2][2];

        gup(ip, 2, 0) = gup(ip, 0, 2);
        gup(ip, 2, 1) = gup(ip, 1, 2);
        gup(ip, 2, 2) = jac[2][0] * jac[2][0] + jac[2][1] * jac[2][1] +
                        jac[2][2] * jac[2][2];

        // the covariant is the inverse of the contravariant by definition
        // gUpper is symmetric
        const ftype inv_detj =
            ftype(1.0) / (gup(ip, 0, 0) * (gup(ip, 1, 1) * gup(ip, 2, 2) -
                                           gup(ip, 1, 2) * gup(ip, 1, 2)) -
                          gup(ip, 0, 1) * (gup(ip, 0, 1) * gup(ip, 2, 2) -
                                           gup(ip, 1, 2) * gup(ip, 0, 2)) +
                          gup(ip, 0, 2) * (gup(ip, 0, 1) * gup(ip, 1, 2) -
                                           gup(ip, 1, 1) * gup(ip, 0, 2)));

        glo(ip, 0, 0) = inv_detj * (gup(ip, 1, 1) * gup(ip, 2, 2) -
                                    gup(ip, 1, 2) * gup(ip, 1, 2));
        glo(ip, 0, 1) = inv_detj * (gup(ip, 0, 2) * gup(ip, 1, 2) -
                                    gup(ip, 0, 1) * gup(ip, 2, 2));
        glo(ip, 0, 2) = inv_detj * (gup(ip, 0, 1) * gup(ip, 1, 2) -
                                    gup(ip, 0, 2) * gup(ip, 1, 1));

        glo(ip, 1, 0) = glo(ip, 0, 1);
        glo(ip, 1, 1) = inv_detj * (gup(ip, 0, 0) * gup(ip, 2, 2) -
                                    gup(ip, 0, 2) * gup(ip, 0, 2));
        glo(ip, 1, 2) = inv_detj * (gup(ip, 0, 2) * gup(ip, 0, 1) -
                                    gup(ip, 0, 0) * gup(ip, 1, 2));

        glo(ip, 2, 0) = glo(ip, 0, 2);
        glo(ip, 2, 1) = glo(ip, 1, 2);
        glo(ip, 2, 2) = inv_detj * (gup(ip, 0, 0) * gup(ip, 1, 1) -
                                    gup(ip, 0, 1) * gup(ip, 0, 1));
    }
}
...
template <typename AlgTraits,
          ...
          typename CoordViewType,
          typename OutputViewType>
void generic_determinant_3d(GradViewType referenceGradWeights,
...
template <typename AlgTraits,
          ...
          typename CoordViewType,
          typename OutputViewType>
void generic_determinant_2d(GradViewType referenceGradWeights,
                            CoordViewType coords,
                            OutputViewType detj)
{
    using ftype = typename CoordViewType::value_type;
