...
RealType hex_volume_grandy(RealType scvcoords[8][3])
{
    ...
    constexpr int face_nodes[nFaces][nodesPerFace] = {{0, 3, 2, 1},
                                                      {4, 5, 6, 7},
                                                      {0, 1, 5, 4},
                                                      {2, 3, 7, 6},
                                                      {1, 2, 6, 5},
    ...
    for (int k = 0; k < nFaces; ++k)
    {
        ...
        for (int d = 0; d < dim; ++d)
        {
            coordv[coordIndex][d] =
                0.25 *
                (coordv[face_nodes[k][0]][d] + coordv[face_nodes[k][1]][d] +
    ...
    for (int k = 0; k < nTri; ++k)
    {
        ...
        dxv[0] =
            (coordv[q][YC] - coordv[p][YC]) * (coordv[r][ZC] - coordv[p][ZC]) -
            (coordv[r][YC] - coordv[p][YC]) * (coordv[q][ZC] - coordv[p][ZC]);

        dxv[1] =
            (coordv[r][XC] - coordv[p][XC]) * (coordv[q][ZC] - coordv[p][ZC]) -
            (coordv[q][XC] - coordv[p][XC]) * (coordv[r][ZC] - coordv[p][ZC]);

        dxv[2] =
            (coordv[q][XC] - coordv[p][XC]) * (coordv[r][YC] - coordv[p][YC]) -
            (coordv[r][XC] - coordv[p][XC]) * (coordv[q][YC] - coordv[p][YC]);

        volume += triFaceMid[0] * dxv[0] + triFaceMid[1] * dxv[1] +
                  triFaceMid[2] * dxv[2];
    }
...
void subdivide_hex_8(CoordViewType coords,
                     typename CoordViewType::value_type coordv[27][3])
{
    ...
    // what is done for a multilinear Hex27 element, which has equivalent nodal
    // locations.
    for (int d = 0; d < 3; ++d)
    {
        // Face 0
