...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void invert_matrix33(const ScalarType A[3][3],
                                                 ScalarType Ainv[3][3])
{
...
invert_matrix33(const ScalarType* POINTER_RESTRICT A,
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void solve22(const ScalarType* POINTER_RESTRICT A,
                                         const ScalarType* POINTER_RESTRICT b,
                                         ScalarType* POINTER_RESTRICT x)
{
    STK_ThrowAssert(
        stk::simd::are_any(determinant22(A) > tiny_positive_value()));
...
// computes b = A^-1 x;
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void solve33(const ScalarType* POINTER_RESTRICT A,
                                         const ScalarType* POINTER_RESTRICT b,
                                         ScalarType* POINTER_RESTRICT x)
{
    STK_ThrowAssert(
        stk::simd::are_any(determinant33(A) > tiny_positive_value()));
    ...
    x[X_RANK1] = ((A[YY] * A[ZZ] - A[YZ] * A[ZY]) * b[X_RANK1] +
                  (A[XZ] * A[ZY] - A[XY] * A[ZZ]) * b[Y_RANK1] +
                  (A[XY] * A[YZ] - A[XZ] * A[YY]) * b[Z_RANK1]) *
                 inv_detA;

    x[Y_RANK1] = ((A[YZ] * A[ZX] - A[YX] * A[ZZ]) * b[X_RANK1] +
                  (A[XX] * A[ZZ] - A[XZ] * A[ZX]) * b[Y_RANK1] +
                  (A[XZ] * A[YX] - A[XX] * A[YZ]) * b[Z_RANK1]) *
                 inv_detA;

    x[Z_RANK1] = ((A[YX] * A[ZY] - A[YY] * A[ZX]) * b[X_RANK1] +
                  (A[XY] * A[ZX] - A[XX] * A[ZY]) * b[Y_RANK1] +
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void
matvec22(const ScalarType* A, const ScalarType* x, ScalarType* b)
{
    b[0] = A[0] * x[0] + A[1] * x[1];
    b[1] = A[2] * x[0] + A[3] * x[1];
}
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void
mxm22(const ScalarType* A, const ScalarType* B, ScalarType* C)
{
    C[0] = A[0] * B[0] + A[1] * B[2];
    C[1] = A[0] * B[1] + A[1] * B[3];
    C[2] = A[2] * B[0] + A[3] * B[2];
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void
mxm33(const ScalarType* A, const ScalarType* B, ScalarType* C)
{
    enum
    {
        XX = 0,
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void
matvec33(const ScalarType* A, const ScalarType* x, ScalarType* b)
{
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void transpose22(const ScalarType* A,
                                             ScalarType* At)
{
    At[0] = A[0];
    At[1] = A[2];
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void transpose33(const ScalarType* A,
                                             ScalarType* At)
{
    enum
    {
