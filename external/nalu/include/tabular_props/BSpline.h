...
public:
    ...
    /** Query the dimensionality of this spline (number of independent
     * variables) */
...
public:
    ...
    inline std::vector<double>& get_control_pts()
    {
        return controlPts_;
    }

    inline const std::vector<double>& get_knot_vector() const
    {
        return knots_;
    };

    inline int get_npts() const
    {
        return npts_;
    }

    inline double get_maxval() const
    {
        return maxIndepVarVal_;
    }

    inline double get_minval() const
    {
        return minIndepVarVal_;
    }

    inline const std::vector<double>& get_basisfun() const
    {
        return basisFun_;
    }
    ...
    {
        return (a.npts_ == npts_ && a.maxIndepVarVal_ == maxIndepVarVal_ &&
...
public:
     ...
     * the mesh implied by indepVars1 and indepVars2, with fortran-style
    ...
    {
        bool isEqual = true;
        std::vector<const BSpline1D*>::const_iterator isp =
            dim2Splines_.begin();
        std::vector<const BSpline1D*>::const_iterator ispa =
            a.dim2Splines_.begin();
...
public:
     ...
     * @param phi : value of the dependent variable at all points on the mesh
     *    implied by x1,x2,x3,x4.  By convention, phi should be arranged in
     * memory such that it varies fastest in x1 and slowest in x4.  The size of
     * phi should be equal to the product of sizes of x1, x2, x3, x4.
     */
