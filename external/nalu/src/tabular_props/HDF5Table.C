...
//============================================================================
HDF5Table::HDF5Table()
    : fileIO_(), indVarSize_(0), dimension_(0), valueMin_(0.0), valueMax_(0.0),
      spline_(), clipEventLogSize_(10), numClipped_(0)
{
}

//============================================================================
HDF5Table::HDF5Table(H5IO* fileIO,
                     std::string tablePropName,
                     std::vector<std::string>& indVarNameVec,
                     std::vector<std::string>& indVarTableNameVec)
    : fileIO_(fileIO), tablePropName_(tablePropName),
      indVarTableNameVec_(indVarTableNameVec),
      indVarSize_(indVarNameVec.size()), dimension_(0), valueMin_(0.0),
      valueMax_(0.0), spline_(NULL), clipEventLogSize_(10), numClipped_(0)
{
    // extract the independent fields; check if there is one..
    if (indVarSize_ == 0)
        throw std::runtime_error(
            "HDF5Table: independent variable size is zero:");
...
//----------------------------------------------------------------------------
void HDF5Table::add_converter(const Converter* converter)
{
...
//----------------------------------------------------------------------------
void HDF5Table::update_input_mapping()
{
    ...
    for (unsigned int i = 0; i < converters_.size(); ++i)
    {
        ...
        if (idx < 0)
        {
            ...
            errmsg << "ERROR: Unable to find the Converter output variable '"
        ...
        {
            std::ostringstream errmsg;
            errmsg << "ERROR: You may not hook the output of more than one"
                   << std::endl
                   << "       Converter to the same Table variable.  Error"
                   << std::endl
    ...
    {
        std::ostringstream errmsg;
        errmsg << "ERROR: The number of input variables needed by HDF5Table "
               << std::endl
               << "is not the same as that defined by indVarTableNameVec_."
               << std::endl;
        throw std::runtime_error(errmsg.str());
    }
    ...
    for (unsigned int i = 0; i < inputNames_.size(); i++)
    {
        ...
        {
            std::ostringstream errmsg;
            errmsg << "ERROR: Unable to find inputNames_ = " << inputNames_[i]
                   << std::endl
...
//----------------------------------------------------------------------------
double HDF5Table::query(const std::vector<double>& inputs) const
{
    ...
    else
    {
        ...
        for (unsigned int i = 0; i < converters_.size(); ++i, ++tableIdx)
        {
            ...
            lookupBuffer_[convTableIndex_[i]] =
                converters_[i]->query(converterBuf_);
        }
    }
    ...
    for (unsigned int i = 0; i < dimension_; ++i)
    {
        ...
        {
            lookupBufferChecked_[i] =
                std::log(std::max(lookupBufferChecked_[i], 1.e-16));
        }
    }
...
//----------------------------------------------------------------------------
double HDF5Table::raw_query(const std::vector<double>& inputs) const
{
    if (converters_.size() == 0)
    {
...
//--------------------------------------------------------------------
void HDF5Table::set_clipping_log_size(unsigned int size)
{
    clipEventLogSize_ = size;
}

//--------------------------------------------------------------------
unsigned int HDF5Table::num_clipping_events() const
{
    return numClipped_;
}

//--------------------------------------------------------------------
const ClipEventLog& HDF5Table::clipping_event_log() const
{
    return clipEventLog_;
}

//--------------------------------------------------------------------
const std::vector<double>& HDF5Table::clipping_event_min_bounds() const
{
    return inputMin_;
}

//--------------------------------------------------------------------
const std::vector<double>& HDF5Table::clipping_event_max_bounds() const
{
    return inputMax_;
}

//--------------------------------------------------------------------
void HDF5Table::clear_clipping_log()
{
    numClipped_ = 0;
    clipEventLog_.clear();
}
...
//----------------------------------------------------------------------------
void HDF5Table::log_clip_event(const std::vector<double>& values) const
{
    double sev = 1.0;
    for (unsigned int i = 0; i < inputMin_.size(); ++i)
    {
        if (values[i] < inputMin_[i])
        {
            // Below lower limit
            sev *= 1.0 +
                   (inputMin_[i] - values[i]) / (inputMax_[i] - inputMin_[i]);
        }
        else if (values[i] > inputMax_[i])
        {
            // Above upper limit
            sev *= 1.0 +
                   (values[i] - inputMax_[i]) / (inputMax_[i] - inputMin_[i]);
        }
    }
    ClipEvent event;
    event.severity = sev;
...
//--------------------------------------------------------------------
bool HDF5Table::has_attribute(const std::string& name) const
{
...
//--------------------------------------------------------------------
const std::string& HDF5Table::attribute(const std::string& name) const
{
    Attributes::const_iterator iatr;
    iatr = attributes_.find(name);
...
//--------------------------------------------------------------------
void HDF5Table::read_hdf5_property()
{
    // Won't need H5IO to be passed, instead get it in the constructor at the
    // file level
...
//============================================================================
//--------------------------------------------------------------------
void HDF5Table::read_hdf5_table(H5IO& io)
{
    //
