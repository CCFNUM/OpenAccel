...
//----------------------------------------------------------------------------
void H5IO::create_file(const std::string& name, int version)
{
    if (file_ >= 0)
    {
        close_file();
...
//----------------------------------------------------------------------------
void H5IO::open_file(const std::string& name)
{
    if (file_ >= 0)
    {
        close_file();
...
//----------------------------------------------------------------------------
H5IO H5IO::create_group(const std::string& name)
{
    if (file_ < 0)
    {
        ostringstream errmsg;
...
//----------------------------------------------------------------------------
H5IO H5IO::open_group(const std::string& name)
{
    if (file_ < 0)
    {
        ostringstream errmsg;
...
//----------------------------------------------------------------------------
unsigned int H5IO::num_attributes()
{
...
//----------------------------------------------------------------------------
void H5IO::h5io_create_group(const std::string& name)
{
    hid_t newGroup = -1;
    ...
    newGroup =
        H5Gcreate(file_, newGroupName.c_str(), 0, H5P_DEFAULT, H5P_DEFAULT);

    if (newGroup < 0)
    {
        ostringstream errmsg;
        errmsg << "ERROR: Could not create HDF5 group named '" << name << "'"
               << endl
               << "       in file '" << fileName_ << "'" << endl;
        throw std::runtime_error(errmsg.str());
    }
...
//----------------------------------------------------------------------------
void H5IO::h5io_open_group(const std::string& name)
{
    hid_t newGroup = -1;
    std::string newGroupName;
    newGroupName = (name.at(0) == '/') ? name                     // Absolute
    ...
    {
        ostringstream errmsg;
        errmsg << "ERROR: Could not open HDF5 group named '" << name << "'"
               << endl
               << "       in file '" << fileName_ << "'" << endl;
        throw std::runtime_error(errmsg.str());
    }
...
//----------------------------------------------------------------------------
hid_t H5IO::h5io_create_scalar()
{
    hid_t space_id = H5Screate(H5S_SCALAR);
    if (space_id < 0)
    {
        ostringstream errmsg;
        errmsg << "ERROR: Could not create scalar dataspace in HDF5 group"
               << endl
...
//----------------------------------------------------------------------------
hid_t H5IO::h5io_create_1D_array(unsigned int size)
{
    hsize_t hsize = size;
    ...
    {
        ostringstream errmsg;
        errmsg << "ERROR: Could not create 1D array dataspace in HDF5 group"
               << endl
               << "       '" << groupName_ << "' in file '" << fileName_ << "'"
               << endl;
        throw std::runtime_error(errmsg.str());
...
//----------------------------------------------------------------------------
hid_t H5IO::h5io_create_attribute(const std::string& name,
                                  hid_t type,
                                  hid_t space)
{
    hid_t attr_id =
        H5Acreate(group_, name.c_str(), type, space, H5P_DEFAULT, H5P_DEFAULT);
    if (attr_id < 0)
    {
        ostringstream errmsg;
        errmsg << "ERROR: Could not create attribute '" << name << "' in"
               << endl
               << "       HDF5 group '" << groupName_ << "' in file '"
               << fileName_ << "'" << endl;
        throw std::runtime_error(errmsg.str());
    }
    return attr_id;
...
//----------------------------------------------------------------------------
hid_t H5IO::h5io_create_dataset(const std::string& name,
                                hid_t type,
                                hid_t space)
{
    hid_t data_id = H5Dcreate(group_,
                              name.c_str(),
                              type,
                              space,
                              H5P_DEFAULT,
...
//----------------------------------------------------------------------------
void H5IO::write_attribute(const std::string& name, int value)
{
    h5io_open_group();
...
//----------------------------------------------------------------------------
void H5IO::write_attribute(const std::string& name, unsigned int value)
{
    h5io_open_group();
    hid_t space_id = h5io_create_scalar();
...
//----------------------------------------------------------------------------
void H5IO::write_attribute(const std::string& name, double value)
{
    h5io_open_group();
    hid_t space_id = h5io_create_scalar();
...
//----------------------------------------------------------------------------
void H5IO::write_attribute(const std::string& name, const std::string& value)
{
    h5io_open_group();
    hid_t space_id = h5io_create_scalar();
...
//----------------------------------------------------------------------------
void H5IO::write_attribute(const std::string& name,
...
//----------------------------------------------------------------------------
bool H5IO::has_attribute(const std::string& name)
{
...
//----------------------------------------------------------------------------
void H5IO::read_attribute(const std::string& name, int& value)
{
    h5io_open_group();
...
//----------------------------------------------------------------------------
void H5IO::read_attribute(const std::string& name, unsigned int& value)
{
    h5io_open_group();
    hid_t attr_id = H5Aopen_name(group_, name.c_str());
...
//----------------------------------------------------------------------------
void H5IO::read_attribute(const std::string& name, double& value)
{
    h5io_open_group();
    hid_t attr_id = H5Aopen_name(group_, name.c_str());
...
//----------------------------------------------------------------------------
void H5IO::read_attribute(const std::string& name, std::string& value)
{
    h5io_open_group();
    hid_t attr_id = H5Aopen_name(group_, name.c_str());
...
//----------------------------------------------------------------------------
void H5IO::read_attribute(unsigned int index,
                          std::string& name,
                          std::string& value)
{
    const size_t NAMESIZE = 128;
...
//----------------------------------------------------------------------------
void H5IO::read_attribute(const std::string& name, std::vector<int>& value)
{
    h5io_open_group();
    hid_t attr_id = H5Aopen_name(group_, name.c_str());
...
//----------------------------------------------------------------------------
void H5IO::read_attribute(const std::string& name,
...
//----------------------------------------------------------------------------
void H5IO::read_attribute(const std::string& name, std::vector<double>& value)
{
    h5io_open_group();
    hid_t attr_id = H5Aopen_name(group_, name.c_str());
    hid_t space_id = H5Aget_space(attr_id);
...
//----------------------------------------------------------------------------
void H5IO::write_dataset(const std::string& name,
                         const std::vector<double>& value)
{
    h5io_open_group();
    hid_t space_id = h5io_create_1D_array(value.size());
    ...
    herr_t err = H5Dwrite(
        data_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, &value[0]);
    if (err < 0)
    {
        ostringstream errmsg;
...
//----------------------------------------------------------------------------
void H5IO::read_dataset(const std::string& name, std::vector<double>& value)
{
    h5io_open_group();
