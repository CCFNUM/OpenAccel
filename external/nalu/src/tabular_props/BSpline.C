              ...
              const vector<double>& U) // knot vector
{
    ...
    if (u <= U[0])
        return p;
    if (u >= U[n + 1])
        return n - 1; // Special case, causing trouble for basis func.
            ...
            const double minIndepVarVal)
{
    ...
    {
        uk.push_back(
            get_uk(indepVars[i], maxIndepVarVal, minIndepVarVal, false));
    }
}
...
BSpline::BSpline(const int order, const int dimension, const bool doClip)
    : order_(order), dim_(dimension), enableValueClipping_(doClip)
{
}
...
BSpline1D::BSpline1D(const int order,
    ...
    : BSpline(order, 1, allowClipping), npts_(indepVars.size()),
...
BSpline1D::BSpline1D(const bool allowClipping)
    : BSpline(0, 1, allowClipping), npts_(0), maxIndepVarVal_(0.0),
      minIndepVarVal_(0.0)
{
}
...
void BSpline1D::dump()
{
    ...
    cout << "-------------------------------------------------------" << endl
         << " order: " << order_ << ",  npts: " << npts_
         << ", nknots: " << (int)knots_.size()
...
//--------------------------------------------------------------------
void BSpline1D::compute_control_pts(const vector<double>& indepVars,
                                    const vector<double>& depVars)
{
    vector<double> uk;
...
//--------------------------------------------------------------------
void BSpline1D::sort_inputs(const std::vector<double>& indepVars,
                            ...
                            std::vector<double>& sortedDepVars) const
{
    ...
    std::vector<double>::iterator iu =
        std::unique(sortedIndepVars.begin(), sortedIndepVars.end());
    sortedIndepVars.erase(iu, sortedIndepVars.end());

    if (indepVars.size() != sortedIndepVars.size())
    {
        std::ostringstream errmsg;
        errmsg << "ERROR: Duplicate entries found in BSpline1D independent"
               << std::endl
               << "       variable vector:" << std::endl
               << std::endl;
    ...
    else
    {
        ...
        else
        {
            ...
            std::ostringstream errmsg;
            errmsg << "ERROR: Non-monotonic independent variable vector found"
                   << std::endl
                   << "       in BSpline1D:" << std::endl
                   << std::endl;
            for (unsigned int i = 0; i < indepVars.size(); ++i)
            {
                errmsg << "       indepVar[" << i << "] = " << indepVars[i]
...
//--------------------------------------------------------------------
double BSpline1D::value(const double* indepVar) const
{
    double result = 0.0;

    // obtain the parametric value for the independent variable
    const double uk = get_uk(
        indepVar[0], maxIndepVarVal_, minIndepVarVal_, enableValueClipping_);
...
//--------------------------------------------------------------------
void BSpline1D::write_hdf5(H5IO& io) const
{
...
//--------------------------------------------------------------------
void BSpline1D::read_hdf5(H5IO& io)
{
    ...
    // Initialize the last few remaining class members from the data we just
    // read
...
BSpline2D::BSpline2D(const int order,
                     ...
                     const std::vector<double>& depVars,
                     const bool allowClipping)
    : BSpline(order, 2, allowClipping), sp1_(NULL)
{
    ...
    {
        std::ostringstream errmsg;
        errmsg << "ERROR in BSpline2d::BSpline2d()!  Inconsistent "
                  "dimensionality between"
               << std::endl
               << "      independent (" << n1n2 << ") and dependent (" << nn
               << ") variables" << std::endl;
        throw std::runtime_error(errmsg.str());
    }
...
BSpline2D::BSpline2D(const bool allowClipping)
    : BSpline(0, 2, allowClipping), sp1_(NULL)
{
}
...
//--------------------------------------------------------------------
void BSpline2D::compute_control_pts(const std::vector<double>& indepVars1,
                                    const std::vector<double>& indepVars2,
                                    const std::vector<double>& depVars)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        BSpline1D* sp1d =
            new BSpline1D(order_, indepVars2, dpvrs, enableValueClipping_);
        dim2Splines_.push_back(sp1d);
    }
}

//--------------------------------------------------------------------
double BSpline2D::value(const double* indepVar) const
{
    //   Q   = sum_j N_j(v) R_j
    ...
    /*
    // here is the brute-force way (compute all control points in the 1-D
    interpolant) vector<BSpline1D*>::const_iterator jj = dim2Splines_.begin();
...
//--------------------------------------------------------------------
void BSpline2D::write_hdf5(H5IO& io) const
{
...
//--------------------------------------------------------------------
void BSpline2D::read_hdf5(H5IO& io)
{
    unsigned int nsp = 0;
...
BSpline3D::BSpline3D(const int order,
                     ...
                     const bool allowClipping)
    : BSpline(order, 3, allowClipping), n1_((int)x1.size()),
      n2_((int)x2.size()), n3_((int)x3.size()), sp1_(NULL)
{
    ...
    {
        std::ostringstream errmsg;
        errmsg << "ERROR in BSpline3D::BSpline3D()!  Inconsistent "
                  "dimensionality between"
               << std::endl
               << "      independent (" << n1_ * n2_ * n3_
               << ") and dependent (" << nn << ") variables" << std::endl;
        throw std::runtime_error(errmsg.str());
    }
...
BSpline3D::BSpline3D(const bool allowClipping)
    : BSpline(0, 3, allowClipping), n1_(0), n2_(0), n3_(0), sp1_(NULL)
{
}

//--------------------------------------------------------------------
BSpline3D::BSpline3D(const BSpline3D& src)
    : BSpline(src.order_, 3, src.enableValueClipping_), n1_(src.n1_),
      n2_(src.n2_), n3_(src.n3_)
{
    // deep copy
    sp1_ = new BSpline1D(*(src.sp1_));
...
//--------------------------------------------------------------------
void BSpline3D::compute_control_pts(const std::vector<double>& x1,
                                    const std::vector<double>& x2,
                                    const std::vector<double>& x3,
                                    const std::vector<double>& phi)
{
    ...
    for (int i = 0; i < n1_; i++)
    {
        ...
        BSpline2D* sp =
            new BSpline2D(order_, x2, x3, dpvrs, enableValueClipping_);
        sp2d_.push_back(sp);
    }
}

//--------------------------------------------------------------------
double BSpline3D::value(const double* x) const
{
    //     Q   = \sum_i N_i(u) R_{ijk}
    ...
    // that will actually be used by the subsequent 1-D interpolation.
    const int p = sp1_->get_order();
    const int ix = find_indx(
        sp1_->get_npts(),
        sp1_->get_order(),
        get_uk(
            x[0], sp1_->get_maxval(), sp1_->get_minval(), enableValueClipping_),
        sp1_->get_knot_vector());
...
//--------------------------------------------------------------------
void BSpline3D::write_hdf5(H5IO& io) const
{
...
//--------------------------------------------------------------------
void BSpline3D::read_hdf5(H5IO& io)
{
    unsigned int nsp = 0;
...
BSpline4D::BSpline4D(const int order,
                     ...
                     const std::vector<double>& phi,
                     const bool allowClipping)
    : BSpline(order, 4, allowClipping), n1_((int)x1.size()),
      n2_((int)x2.size()), n3_((int)x3.size()), n4_((int)x4.size()), sp1_(NULL)
{
    if (n1_ * n2_ * n3_ * n4_ != (int)phi.size())
    {
        std::ostringstream errmsg;
        errmsg << "ERROR in BSpline4D::BSpline4D()!  Inconsistent "
                  "dimensionality between"
               << std::endl
               << "      independent (" << n1_ * n2_ * n3_ * n4_
               << ") and dependent (" << phi.size() << ") variables"
               << std::endl;
        throw std::runtime_error(errmsg.str());
    }
...
BSpline4D::BSpline4D(const bool allowClipping)
    : BSpline(0, 4, allowClipping), n1_(0), n2_(0), n3_(0), n4_(0), sp1_(NULL)
{
}

//--------------------------------------------------------------------
BSpline4D::BSpline4D(const BSpline4D& src)
    : BSpline(src.order_, 4, src.enableValueClipping_), n1_(src.n1_),
      n2_(src.n2_), n3_(src.n3_), n4_(src.n4_)
{
    // deep copy
    sp1_ = new BSpline1D(*(src.sp1_));
...
//--------------------------------------------------------------------
void BSpline4D::compute_control_pts(const std::vector<double>& x1,
                                    const std::vector<double>& x2,
                                    const std::vector<double>& x3,
                                    const std::vector<double>& x4,
                                    const std::vector<double>& phi)
{
    ...
    for (int l = 0; l < n4_; l++)
    {
        for (int k = 0; k < n3_; k++)
        {
            for (int j = 0; j < n2_; j++)
            {
                ...
                if (first)
                {
                    // create the 1D-spline to be used for the first dimension
                    // in lookups later.  Note that we can do this because the
                    // knot sequence is constant across all lines in a given
                    // direction. the control points will change, however....
                    sp1_ = new BSpline1D(sp);
                    first = false;
                }
            }
    ...
    for (int i = 0; i < n1_; i++)
    {
        ...
        // spline and store this 2D surface spline.
        BSpline3D* sp =
            new BSpline3D(order_, x2, x3, x4, dpvrs, enableValueClipping_);
        sp3d_.push_back(sp);
    }
}

//--------------------------------------------------------------------
double BSpline4D::value(const double* x) const
{
    //       Q  = \sum_i N_i(u) R_{ijkl}
...
//--------------------------------------------------------------------
void BSpline4D::write_hdf5(H5IO& io) const
{
...
//--------------------------------------------------------------------
void BSpline4D::read_hdf5(H5IO& io)
{
    unsigned int nsp = 0;
...
BSpline5D::BSpline5D(const int order,
                     ...
                     const std::vector<double>& phi,
                     const bool allowClipping)
    : BSpline(order, 5, allowClipping), n1_((int)x1.size()),
      n2_((int)x2.size()), n3_((int)x3.size()), n4_((int)x4.size()),
      n5_((int)x5.size()), sp1_(NULL)
{
    if (n1_ * n2_ * n3_ * n4_ * n5_ != (int)phi.size())
    {
        std::ostringstream errmsg;
        errmsg << "ERROR in BSpline5D::BSpline5D()!  Inconsistent "
                  "dimensionality between"
               << std::endl
               << "      independent (" << n1_ * n2_ * n3_ * n4_ * n5_
               << ") and dependent (" << phi.size() << ") variables"
               << std::endl;
        throw std::runtime_error(errmsg.str());
    }
...
BSpline5D::BSpline5D(const bool allowClipping)
    : BSpline(0, 5), n1_(0), n2_(0), n3_(0), n4_(0), n5_(0), sp1_(NULL)
{
}

//--------------------------------------------------------------------
BSpline5D::BSpline5D(const BSpline5D& src)
    : BSpline(src.order_, 5, src.enableValueClipping_), n1_(src.n1_),
      n2_(src.n2_), n3_(src.n3_), n4_(src.n4_), n5_(src.n5_)
{
    // deep copy
    sp1_ = new BSpline1D(*(src.sp1_));
...
//--------------------------------------------------------------------
void BSpline5D::compute_control_pts(const std::vector<double>& x1,
                                    const std::vector<double>& x2,
                                    const std::vector<double>& x3,
                                    const std::vector<double>& x4,
                                    const std::vector<double>& x5,
                                    const std::vector<double>& phi)
{
        ...
        for (int l = 0; l < n4_; l++)
        {
            ...
            for (int k = 0; k < n3_; k++)
            {
                ...
                for (int j = 0; j < n2_; j++)
                {
                    ...
                    if (first)
                    {
                        // create the 1D-spline to be used for the first
                        // dimension in lookups later.  Note that we can do this
                        // because the knot sequence is constant across all
                        // lines in a given direction. the control points will
                        // change, however....
                        sp1_ = new BSpline1D(sp);
                        first = false;
                    }
    ...
    for (int i = 0; i < n1_; i++)
    {
            ...
            for (int l = 0; l < n4_; l++)
            {
                for (int k = 0; k < n3_; k++)
                {
                    ...
                    for (int j = 0; j < n2_; j++)
                    {
                        ...
                        if (j + shift > nRR - 1)
                            std::cout << "ERROR2 on INDEX" << std::endl;
        ...
        BSpline4D* sp =
            new BSpline4D(order_, x2, x3, x4, x5, dpvrs, enableValueClipping_);
        sp4d_.push_back(sp);
    }
}

//--------------------------------------------------------------------
double BSpline5D::value(const double* x) const
{
    //       Q  = \sum_i N_i(u) R_{ijkl}
    // R_{ijkl} = \sum_j N_j(v) \sum_k N_k(w) P_{ijkl}
...
//--------------------------------------------------------------------
void BSpline5D::write_hdf5(H5IO& io) const
{
...
//--------------------------------------------------------------------
void BSpline5D::read_hdf5(H5IO& io)
{
    unsigned int nsp = 0;
