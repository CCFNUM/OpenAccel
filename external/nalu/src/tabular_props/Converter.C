...
//============================================================================
Converter* ConverterFactory::create(const std::string& converterType) const
{
    ...
    {
        throw std::runtime_error("Unrecognized Converter type '" +
                                 converterType + "'.");
    }
...
Converter::Converter(const std::string& converterType)
    : dimension_(0), converterType_(converterType)
{
}

//----------------------------------------------------------------------------
void Converter::print_summary() const
{
    ...
    //
    unsigned int total_width = type_width + 2 + input_width + 3 + output_width +
                               3; // Content + margins/dividers
    ...
    // Print the first row of the table, which will have an entry for all
    // columns
    //
...
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
void Converter::read_hdf5(H5IO& io)
{
    //
    ...
    if (typeInFile != converterType_)
    {
        ...
        errmsg << "ERROR: Attempted to read a Converter of type '" << typeInFile
               << "'" << std::endl
...
//============================================================================
NameConverter::NameConverter() : Converter("NameConverter")
{
...
//----------------------------------------------------------------------------
double NameConverter::query(const std::vector<double>& inputs) const
{
...
ChiConverter::ChiConverter()
    : Converter("ChiConverter"), zStoich_(-1.0), fChiMeanTable_(NULL)
{
    // This default constructor is intended to be used when reading data in
    // from an HDF5 file.
}
...
//----------------------------------------------------------------------------
double ChiConverter::query(const std::vector<double>& inputs) const
{
...
//----------------------------------------------------------------------------
void ChiConverter::read_hdf5(H5IO& io)
{
...
DeltaChiConverter::DeltaChiConverter()
    : Converter("DeltaChiConverter"), zStoich_(-1.0)
{
    // This default constructor is intended to be used when reading data in
    // from an HDF5 file.
}
...
//----------------------------------------------------------------------------
double DeltaChiConverter::query(const std::vector<double>& inputs) const
{
    // Note that the inputs will always contain these variables:
    //
    //   inputs[0] = ZMean
...
//----------------------------------------------------------------------------
void DeltaChiConverter::read_hdf5(H5IO& io)
{
    // Make sure that the base class pieces get read
    Converter::read_hdf5(io);
...
GammaConverter::GammaConverter()
    : Converter("GammaConverter"), numTableInputs_(0), fGammaMeanTable_(NULL)
{
    // This default constructor is intended to be used when reading data in
    // from an HDF5 file.
}
...
//----------------------------------------------------------------------------
double GammaConverter::query(const std::vector<double>& inputs) const
{
    // Note that the inputs will always contain these variables:
    //
    //   inputs[0]                = ZMean
...
//----------------------------------------------------------------------------
void GammaConverter::read_hdf5(H5IO& io)
{
    // Make sure that the base class pieces get read
    Converter::read_hdf5(io);
...
DeltaGammaConverter::DeltaGammaConverter()
    : Converter("DeltaGammaConverter"), numMixFrac_(0)
{
    // This default constructor is intended to be used when reading data in
    // from an HDF5 file.
}
...
//----------------------------------------------------------------------------
double DeltaGammaConverter::query(const std::vector<double>& inputs) const
{
    // Note that the inputs will always contain these variables:
    //
    //   inputs[0]             = ZMean[0]
    ...
    const double fGamma =
        std::max(F_gamma(zMean_, zStoich_, gammaMaxStoich_), 1.e-12);

    return gammaMean / fGamma;
}

//----------------------------------------------------------------------------
void DeltaGammaConverter::read_hdf5(H5IO& io)
{
    // Make sure that the base class pieces get read
    Converter::read_hdf5(io);
...
HStarConverter::HStarConverter(const bool isDelta)
    : Converter((isDelta) ? "DeltaHStarConverter" : "HStarConverter"),
      numMixFrac_(0), hStar_ref_min_(0.0)
{
    // This default constructor is intended to be used when reading data in
    // from an HDF5 file.
}
...
//----------------------------------------------------------------------------
double HStarConverter::query(const std::vector<double>& inputs) const
{
    //
...
//----------------------------------------------------------------------------
void HStarConverter::read_hdf5(H5IO& io)
{
    // Make sure that the base class pieces get read
    Converter::read_hdf5(io);
...
//----------------------------------------------------------------------------
void HStarConverter::augmented_mixfrac(const std::vector<double>& mixFrac,
