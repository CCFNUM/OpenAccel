 ...
 */
//============================================================================
double FChi::query(const double Z)
{
    return F_chi(Z);
}
 ...
 */
//============================================================================
double FGamma::query(const double* Z) const
{
...
{
public:
    Coord2D(double xInit = 0.0, double yInit = 0.0) : x(xInit), y(yInit){};
    double x;
    double y;
               ...
               std::vector<double> gamma_st)          // local copy
{
    ...
    else if (Zpoint.size() == 2)
    {
        ...
        // stoichiometric line is extended from that point to the opposite
        // corner, where the gamma value is driven to zero.
        //
        //  Z1                            (*) = Bounding line
        //  |                             (\) = Stoichiometric line
        ...
        const double ZstyxRatio =
            dxZstZero ? 0.0 : (Zst1.y - Zst2.y) / (Zst1.x - Zst2.x);
        const double ZstxyRatio =
            dyZstZero ? 0.0 : (Zst1.x - Zst2.x) / (Zst1.y - Zst2.y);
        ...
        {
            Sx = (Zst2.y - Z.y + Z.x * ZOyxRatio - Zst2.x * ZstyxRatio) /
                 (ZOyxRatio - ZstyxRatio);
        }
        ...
        {
            Sy = (Zst2.x - Z.x + Z.y * ZOxyRatio - Zst2.y * ZstxyRatio) /
                 (ZOxyRatio - ZstxyRatio);
        }
        ...
        {
            Bx = (B2.y - Z.y + Z.x * ZOyxRatio - B2.x * ByxRatio) /
                 (ZOyxRatio - ByxRatio);
        }
        ...
        {
            By = (B2.x - Z.x + Z.y * ZOxyRatio - B2.y * BxyRatio) /
                 (ZOxyRatio - BxyRatio);
        }
    ...
    else
    {
        throw std::runtime_error(
            "No more than two mixture fractions are supported in F_gamma()");
    }
