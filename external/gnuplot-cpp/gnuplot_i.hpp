...
public:
    ...
    ///  csplines, bezier, acsplines (for data values > 0), sbezier, unique,
    ///  frequency
    ...
    /// set isoline density (grid) for plotting functions as surfaces (for 3d
    /// plots)
    ...
    /// position: inside/outside, left/center/right, top/center/bottom,
    /// nobox/box
    ...
    /// turns on/off log scaling for the specified xaxis (logscale is not set by
    /// default)
    Gnuplot& set_xlogscale(const double base = 10);
    /// turns on/off log scaling for the specified yaxis (logscale is not set by
    /// default)
    Gnuplot& set_ylogscale(const double base = 10);
    /// turns on/off log scaling for the specified zaxis (logscale is not set by
    /// default)
    ...
    template <typename X>
    Gnuplot& plot_x(const std::vector<X>& x,
...
//
inline int Gnuplot::tmpfile_num = 0;

#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) ||                 \
    defined(__TOS_WIN__)
inline std::string Gnuplot::m_sGNUPlotFileName = "pgnuplot.exe";
inline std::string Gnuplot::m_sGNUPlotPath = "C:/program files/gnuplot/bin/";
#elif defined(unix) || defined(__unix) || defined(__unix__) ||                 \
    defined(__APPLE__)
inline std::string Gnuplot::m_sGNUPlotFileName = "gnuplot";
inline std::string Gnuplot::m_sGNUPlotPath = "/usr/local/bin/";
#endif

#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) ||                 \
    defined(__TOS_WIN__)
inline std::string Gnuplot::terminal_std = "windows";
#elif (defined(unix) || defined(__unix) || defined(__unix__)) &&               \
    !defined(__APPLE__)
inline std::string Gnuplot::terminal_std = "x11";
#elif defined(__APPLE__)
inline std::string Gnuplot::terminal_std = "aqua";
#endif
...
template <typename X>
inline Gnuplot& Gnuplot::plot_x(const std::vector<X>& x,
                                const std::vector<std::string>& title)
{
...
#elif defined(unix) || defined(__unix) || defined(__unix__) ||                 \
    defined(__APPLE__)
    if (Gnuplot::file_exists(tmp,
                             1)) // check existence and execution permission
#endif
    {
...
inline Gnuplot& Gnuplot::set_style(const std::string& stylestr)
{
    if (stylestr.find("lines") == std::string::npos &&
        ...
        stylestr.find("boxes") ==
            std::string::npos && // 1-4 columns of data are required
        stylestr.find("filledcurves") == std::string::npos &&
        stylestr.find("histograms") ==
            std::string::npos) // only for one data column
    //        stylestr.find("labels")         == std::string::npos  &&  // 3
    //        columns of data are required stylestr.find("xerrorbars")     ==
    //        std::string::npos  &&  // 3-4 columns of data are required
    //        stylestr.find("xerrorlines")    == std::string::npos  &&  // 3-4
    //        columns of data are required stylestr.find("errorbars")      ==
    //        std::string::npos  &&  // 3-4 columns of data are required
    //        stylestr.find("errorlines")     == std::string::npos  &&  // 3-4
    //        columns of data are required stylestr.find("yerrorbars")     ==
    //        std::string::npos  &&  // 3-4 columns of data are required
    //        stylestr.find("yerrorlines")    == std::string::npos  &&  // 3-4
    //        columns of data are required stylestr.find("boxerrorbars")   ==
    //        std::string::npos  &&  // 3-5 columns of data are required
    //        stylestr.find("xyerrorbars")    == std::string::npos  &&  // 4,6,7
    //        columns of data are required stylestr.find("xyerrorlines")   ==
    //        std::string::npos  &&  // 4,6,7 columns of data are required
    //        stylestr.find("boxxyerrorbars") == std::string::npos  &&  // 4,6,7
    //        columns of data are required stylestr.find("financebars")    ==
    //        std::string::npos  &&  // 5 columns of data are required
    //        stylestr.find("candlesticks")   == std::string::npos  &&  // 5
    //        columns of data are required stylestr.find("vectors")        ==
    //        std::string::npos  && stylestr.find("image")          ==
    //        std::string::npos  && stylestr.find("rgbimage")       ==
    //        std::string::npos  && stylestr.find("pm3d")           ==
    //        std::string::npos  )
    {
...
//
inline Gnuplot& Gnuplot::savetofigure(const std::string filename,
...
//
// set the xrange
inline Gnuplot& Gnuplot::set_xrange(const double iFrom, const double iTo)
{
    std::ostringstream cmdstr;
...
// set the yrange
//
inline Gnuplot& Gnuplot::set_yrange(const double iFrom, const double iTo)
{
    std::ostringstream cmdstr;
...
// set the zrange
//
inline Gnuplot& Gnuplot::set_zrange(const double iFrom, const double iTo)
{
    std::ostringstream cmdstr;
...
//
inline Gnuplot& Gnuplot::set_cbrange(const double iFrom, const double iTo)
{
    std::ostringstream cmdstr;
...
inline void Gnuplot::init()
{
    ...
    //
    std::string tmp =
        Gnuplot::m_sGNUPlotPath + "/" + Gnuplot::m_sGNUPlotFileName;
...
#elif defined(unix) || defined(__unix) || defined(__unix__) ||                 \
    defined(__APPLE__)
            if (Gnuplot::file_exists(
                    tmp, 1)) // check existence and execution permission
#endif
            {
