...
class Trilinos(CMakePackage, CudaPackage, ROCmPackage):
    ...
    version("16.2.0",
            sha256=
            "543aa56232d7c0cbe73705fab2d3b5524f11b15fef8917aa14de02d23a5ca418")
    ...
    variant("sierra_migration", default=True, description="Enable Sierra migration")
    ...
    with when("@14.4: +kokkos"):
        ...
        depends_on("kokkos~wrapper", when="~wrapper")
        depends_on("kokkos+cuda_relocatable_device_code~shared",
                   when="+cuda_rdc")
        depends_on("kokkos+hip_relocatable_device_code~shared",
                   when="+rocm_rdc")
    ...
    def cmake_args(self):
        ...
        options.extend(
            [
                ...
                define_from_variant("SIERRA_MIGRATION", "sierra_migration"),
